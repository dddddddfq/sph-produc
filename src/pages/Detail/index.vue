				...
				<div class="InfoWrap">
					...
					<div class="choose">
						<div class="chooseArea">
							...
							<!-- 遍历商品属性的标题 -->
							<dl
								v-for="(spuSaleAttr, index) in spuSaleList"
								:key="spuSaleAttr.id"
							>
								...
								<dd
									...
									:key="item.id"
									@click="changeActive(spuSaleAttr.spuSaleAttrValueList, item)"
								>
...
export default {
	...
	computed: {
		...mapGetters(["categoryView", "skuInfo", "spuSaleList"]),
		skuImageList() {
	...
	methods: {
		...
		async addShopCart() {
			try {
				let result = await this.$store.dispatch("addAndUpdataShopCarts", {
