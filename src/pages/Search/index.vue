        ...
        <div class="details clearfix">
          <div class="sui-navbar">
            <div class="navbar-inner filter">
              <ul class="sui-nav">
                <li :class="{ active: isOne }" @click="changeInfo('1')">
                  <a>综合<span v-show="isOne" class="iconfont" :class="{'icon-todown':isDesc,'icon-up':isAsc}"></span></a>
                </li>
                <li :class="{ active: isTwo }" @click="changeInfo('2')">
                  <a>价格 <span v-show="isTwo" class="iconfont"
                      :class="{'icon-todown':isDesc,'icon-up':isAsc}"></span></a>
                </li>
              </ul>
            </div>
          </div>
            ...
            <ul class="yui3-g">
              <li class="yui3-u-1-5" v-for="(good, index) in goodsList" :key="good.id">
                <div class="list-wrap">
                  ...
                  <div class="commit">
                    <i class="command">已有<span>{{pj()}}</span>人评价</i>
                  </div>
                  <div class="operate">
...
<script>
  ...
  export default {
    ...
    watch: {
      //监听route次数 如果发生了改变哪么就带着改变的参数再次发请求获取数据
      $route(newValue, oldValue) {
        //在发请求之前,接口需要传递参数.进行整理(在发请求之前把参数整理好,服务器就会返回相对应的数据)
        this.searchParams.category1Id = undefined;
        this.searchParams.category2Id = undefined;
        this.searchParams.category3Id = undefined;
